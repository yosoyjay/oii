...
class Node:
    ...
    def isLeaf(self):
        return self.leftChild is None or self.rightChild is None
    def __repr__(self):
        return '(%d %d %d %d)' % (self.left, self.top, self.right, self.bottom)
    def insert(self, w, h):
        if not self.isLeaf():
            newNode = self.leftChild.insert(w,h)
            if newNode is not None:
                return newNode
            return self.rightChild.insert(w,h)
        else:
            if self.occupied:
                return None
            ...
            if dw > dh:
                self.leftChild = Node(self.left, self.top, self.left + w, self.bottom)
                self.rightChild = Node(self.left + w, self.top, self.right, self.bottom)
            else:
                self.leftChild = Node(self.left, self.top, self.right, self.top + h)
                self.rightChild = Node(self.left, self.top + h, self.right, self.bottom)
            
            return self.leftChild.insert(w, h)

        
class JimScottRectanglePacker(RectanglePacker):
    def __init__(self, packingAreaWidth, packingAreaHeight):
        RectanglePacker.__init__(self, packingAreaWidth, packingAreaHeight)
        self.tree = Node(0, 0, packingAreaWidth, packingAreaHeight)

    def TryPack(self, w, h):
        node =  self.tree.insert(w, h)
        if node is not None:
            return Point(node.left, node.top)
