...
def alt(bin_lid):
    ...
    with open(csv_out,'w') as co:
        for row in rows:
            print(','.join([row[n] for n in range(4)]), file=co)
        ...
        if pid == 0: # subprocess
            ...
            if os.path.exists(lightmap):
                ...
                with open(param,'w') as fout:
                    ...
                    print('top %d' % 0, file=fout)
                    if LR == 'L':
                        print('left %d' % 0, file=fout)
                    else:
                        print('left %d' % (w/2), file=fout)
...
def correct(bin_lid,learn_lid=None):
    ...
    for LR in 'LR':
        ...
        with open(param,'w') as fout:
            ...
            print('metafile %s' % re.sub(r'/([^/]+)$',r'/ \1',eic), file=fout)
            print('tmpdir %s' % correct_tmp, file=fout)
            print('load %s' % lightmap, file=fout)
            print('num_to_process %d' % NUM_CORRECT, file=fout)
            for k,v in list(DEFAULT_IC_CONFIG.items()):
                print('%s %s' % (k,str(v)), file=fout)
            print('binary_format', file=fout)
            print('num_rows %d' % h, file=fout)
            ...
            with open(listfile,'w') as fout:
                for f in imgs[n::NUM_PROCS]:
                    ...
                    if not os.path.exists(outtif):
                        print('%s %s' % (intif, outtif), file=fout)
                        count += 1
            if count > 0:
