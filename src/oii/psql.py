...
class PsqlStore(object):
    ...
    def unparse_row(self,record):
        ...
        FIELDS = self.FIELDS
        row = dict(list(zip(FIELDS,[None for _ in range(len(FIELDS))])))
    ...
    def query(self,**template):
        ...
        FIELDS = self.FIELDS
        wc = [(k+'=%s',v) for k,v in list(template.items()) if k in FIELDS]
        if len(wc) > 0:
            (where,values) = (' and '.join(zip(*wc)[0]), list(zip(*wc))[1])
