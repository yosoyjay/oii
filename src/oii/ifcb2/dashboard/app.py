...
@memoize(ttl=30,key=lambda args: frozenset(list(args[0].items())))
...
@memoize(ttl=30,key=lambda args: frozenset(list(args[0].items()) + [args[1]]))
def get_product_file(parsed, product_type):
    parsed = dict(list(parsed.items()))
    time_series = parsed['ts_label']
...
def serve_tags(ts_label, pid):
    ...
    return Response(json.dumps(list(map(str,b.tags))), mimetype=MIME_JSON)
