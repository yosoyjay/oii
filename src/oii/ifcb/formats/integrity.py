...
def check_hdr(hdr_source):
    try:
        ...
        raise IntegrityException('.hdr failed: ' + str(e)).with_traceback(sys.exc_info()[2])
...
def check_adc(adc_source, schema_version=SCHEMA_VERSION_2):
    ...
    try:
        ...
        raise IntegrityException('.adc failed: ' + str(e)).with_traceback(sys.exc_info()[2])
...
def check_roi(roi_source, targets):
    ...
    def read_n(fin, n):
        try:
            ...
            raise IntegrityException('unable to read roi data: '+str(e)).with_traceback(sys.exc_info()[2])
        if len(r) != n:
