...
def flatten(dictlike, include=None, exclude=None):
    ...
    result = dict(list(dictlike.items()))
...
class ScopedExpr(object):
    ...
    def get_list(self,attr_name,delim=None):
        ...
        return [interpolate(t,self.bindings) for t in templates]
    @property
    def tag(self):
        return self.elt.tag
...
def with_distinct(solution_generator,distinct=None):
    ...
    for raw_solution in solution_generator:
        ...
        f_solution = frozenset(list(solution.items()))
...
def with_aliases(solution_generator,aliases={}):
    for raw_solution in solution_generator:
        s = {}
        for k,v in list(raw_solution.items()):
            try:
...
def evaluate_block(exprs,bindings={},global_namespace={}):
    ...
    def local_block(exprs,inner_bindings={}):
        bb = dict(list(bindings.items())) # an appropriate copy operation
    ...
    # </vars>
        ...
        try:
            ...
            if len(sub_val_exprs) == 0:
                var_vals = [interpolate(t,bindings) for t in re.split(delim,expr.raw_text)]
                for s in rest(expr,dict(list(zip(var_names,var_vals)))):
                    yield s
            else:
                for sub_val_expr in sub_val_exprs:
                    sub_val_expr = ScopedExpr(sub_val_expr, bindings)
                    var_vals = [interpolate(t,bindings) for t in re.split(delim,sub_val_expr.raw_text)]
                    for s in rest(expr,dict(list(zip(var_names,var_vals)))):
                        yield s
        except UnboundVariable as uv:
    ...
    # match also acts as an implicit, unnamed block supporting block-level modifiers.
        ...
        if m:
            groups = m.groups()
            named_ixs = list(p.groupindex.values())
...
#            print 'groupdict = %s' % m.groupdict()
            ...
            for name,group in list(m.groupdict().items()):
...
def parse(*ldr_streams):
    ...
    for ldr_stream in ldr_streams:
        ...
        for c in xml.xpath('//comment()'):
            p = c.getparent()
            p.remove(c)
        namespace.update(list(find_names(xml).items()))
    return namespace
