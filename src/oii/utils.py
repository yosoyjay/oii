...
def order_keys(d,s):
    sk = [key for key in s if key in list(d.keys())]
    dk = sorted([key for key in list(d.keys()) if key not in s])
    return sk + dk
...
def mapr(function,item):
    ...
    try:
        return dict([(k,mapr(function,v)) for k,v in item.items()])
    except:
        pass
    try:
        return [mapr(function,i) for i in item]
    except:
        pass
    try:
...
def __struct_wrap(item):
    ...
    try: # assume dict
        ...
        pass
    try: # assume sequence
        return list(map(__struct_wrap,item))
    except TypeError:
        return item
...
def destructs(item):
    ...
    try:
        ...
        pass
    try:
        return dict([(k,destructs(v)) for k,v in item.items()])
    except:
        pass
    try:
        return list(map(destructs,item))
    except:
        pass
    return item
        ...
        for k,v in self.__dict__.items():
            try:
                ...
                if isinstance(v,list):
                    result[k] = [e.destruct if isinstance(e,Struct) else e for e in v]
                elif isinstance(v,tuple):
                    result[k] = tuple([e.destruct if isinstance(e,Struct) else e for e in v])
                else:
                    result[k] = v
        return result
...
def dict_slice(d,schema,default=__none):
    ...
    try: # allow schema to be a dict mapping keys to functions
        if default != __none:
            xf = dict(list(zip(list(schema.keys()),[default for _ in list(schema.keys())])))
        else:
            xf = {}
        xf.update(dict((k,v) for k,v in d.items() if k in list(schema.keys())))
        xf.update(dict((k,schema[k](xf[k])) for k in list(schema.keys()) if schema[k] is not None))
        xf.update(dict((k,d[k]) for k in list(schema.keys()) if schema[k] is None))
        return xf
    except: # ok, schema must be a sequence of keys
        if default != __none:
            xf = dict(list(zip(schema,[default for _ in schema])))
        else:
            xf = {}
        xf.update(dict((k,v) for k,v in d.items() if k in schema))
        return xf

def dict_rename(d,keymap):
    r = {}
    for k1,k2 in list(keymap.items()):
        r[k2] = d[k1]
    return r
...
def asciitable(dicts,disp_cols=None,none_msg=None,border=True):
    ...
    if disp_cols is not None:
        ...
        cols = sorted(list(set(reduce(lambda x,y: x+y, [list(d.keys()) for d in dicts]))))
